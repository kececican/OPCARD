...
*
* # 🏭 Dijital Operasyon Kartı Dashboard

Bu proje, endüstriyel üretim hatlarındaki "Operasyon Kartlarının" (standart iş talimatları) Google Sheets ve Google Apps Script kullanılarak dijitalleştirilmesi, standartlaştırılması ve yönetilmesi amacıyla geliştirilmiştir.

## 🎯 Projenin Amacı
Geleneksel Excel dosyaları üzerinden manuel olarak yürütülen operasyon kartı hazırlama sürecindeki format bozulmaları, fotoğraf boyutlandırma sorunları ve zaman kayıplarını ortadan kaldırmak. Kullanıcılara hatasız, web tabanlı bir veri giriş arayüzü sunarak otomatik standart Excel çıktıları üretmek.

## ✨ Temel Özellikler
* **🔍 Mevcut Durum Görüntüleme:** Sisteme kayıtlı güncel operasyon kartını okuma.
* **✍️ Dinamik Veri Girişi:** Parametreler, makine bilgileri ve işlem adımları için standart web formu.
* **📸 Entegre Fotoğraf Yükleme:** Kameradan veya bilgisayardan seçilen fotoğrafların otomatik Google Drive'a yüklenmesi.
* **👁️ Canlı Excel Ön İzleme:** Veriler kaydedilmeden önce, hücre yerleşimlerinin Excel'deki birebir simülasyonunu gösteren onay ekranı.
* **💾 Otomatik Excel (XLSX) Export:** Onaylanan verilerin arka planda temiz bir şablona işlenip tekil bir Excel dosyası olarak kullanıcıya indirtilmesi.
...
Veritabanı olarak kullanılacak Google Sheets URL'si
Verinin okunacağı ve yazılacağı sekme (Sheet) isimleri
İlgili sekmelerdeki sütun numarası → başlık eşleşmeleri
Valeo logo URL'si (üst barda kullanılacak)
Valeo kurumsal renk skalası (Hex kodları)
